export default class effect {
  ...
  setText( text ) {
    ...
    this.arrayText = [...text].map( ( el, index ) => {
      return { 
        start: index, to: el || '', end: index + floor( random() * this.amount )
      }
    })
  ...
  animate( currentTime = 0 ) {
    ...
    this.arrayText.forEach( el => {
      ...
      if( this.frame >= end + this.duration && el.to === '|' ) outline += '<br>'
        ...
        if( !char || random() < 0.28 ) char = this.char[floor( random() * this.char.length )]
        outline += `<span class="double">${ char }</span>`
      }
    })
